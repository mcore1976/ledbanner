...
int   main(void)
{
	...
	MAX7219_initN(MODULESNUMBER);
        ...
        // convert all 'sentence' characters into bitmaps 8x8 by program memory 
	...
	while (1) 
            ...
            while (offset2 < (BUFLENGTH - (8*MODULESNUMBER)) )
              {
 	            _delay_ms(20);  // scrolling speed - decrease for faster scroll

                     offset2++;    //  current Column index for scrolling
                       ...
                       for(int i = 0; i < 8; i++) 
                         {
                           for(int j = 7; j > -1; j--) 
                           {
                            messagebuf[i+offset] = (messagebuf[i+offset] << 1) | ((MAX7219_state[j+offset+offset2] >> (7 - i)) & 0x01);
                           };  // end of 'j' loop
                          }; // end of 'i'loop
                     ...
                     // sending 8 rows data to N character display from 'messagebuf' array       
                      ...
                      // send buffer content to N modules at once, first module is last in sentence 
                       for (k=0; k < MODULESNUMBER ; k++)
                        {   
                          commands[(k*2)] = row+1;
                          commands[(k*2)+1] = messagebuf[row + ((MODULESNUMBER - k -1 )*8)];
                         };  // end of 'k' loop
